...
class Graph(object):
    ...
    def has_cycle(self): # Uses the dfs method starting at each vertex
    ...
    def get_incoming_vertex(self, v): #Returns list with incoming vertices
        verts = []
        nVert = len(self.Vertices)
        for i in range(nVert):
    ...
    def toposort(self):
        results = []
        no_incoming = Stack()
        for i in range(len(self.Vertices)): # Adds vertices to no_incoming
            if len(self.get_incoming_vertex(i))==0:
                no_incoming.push(i)
        while not no_incoming.is_empty():
            current = no_incoming.pop()
            results.append(self.Vertices[current].label) # Adds current to results
            for i in self.get_adj_vertexes(current):
                self.adjMat[current][i] = 0 #Removes outgoing edges
                if len(self.get_incoming_vertex(i))==0: #Checks if no incoming
                    no_incoming.push(i)
        return results
